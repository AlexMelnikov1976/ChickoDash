...
**Focus:** Phase 2.12 — Owner P&L (вкладка «👑 Owner PnL» в dashboard.html, реальные данные ClickHouse dept_id=42, план/факт/прогноз/сценарий)
...
**Доступ:**
- Флаг `is_owner: true` в KV USERS (`user:<email>`). Сейчас включён только для `melnikov181076@gmail.com`.
- При `is_owner !== true` вкладка Owner PnL в nav скрыта, `/api/owner/*` возвращают 403.
...
## Следующие шаги — post Phase 2.12
...
**Приоритет 3 (Owner PnL):** Убрать устаревший `public/owner-pnl.html` и роут `/owner-pnl` из `src/index.ts`. Вернуть 💼 PnL таб (убрать `style="display:none"` в nav dashboard.html) когда потребуется для команды.
