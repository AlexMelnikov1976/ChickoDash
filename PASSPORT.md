...
**Версия:** v3.37
**Дата обновления:** 24.04.2026, день-9, день  
**Коммит:** (pending) — Phase 2.9.3 closed: admin activity analytics tab
**Focus:** стабилизация staff endpoints на не-Калининградских ресторанах, фикс /api/menu-analysis 400, re-enable AI
...
### Phase 2.9 — Staff Analysis + Admin Analytics ✅ ГОТОВ

Полный раздел «Персонал» для франчайзи + внутренняя аналитика активности для админа. Итог session 9 (23-24.04, день-8/9).

**2.9.0 — Skeleton с моками (commit 520a0f3):**
- 6 endpoints в `src/staff.ts` — list, detail, groups, performance, managers, losses
- Mock-данные, фиксированный список из 25 реальных имён Калининграда
- Вкладка «👥 Персонал» в dashboard, 6 блоков рендера, drawer сотрудника

**2.9.2b — Моки с формулами и порогом left (v2 пакта → v3):**
- Моки пропорциональны `daysBetween(start, end)` — за 7 дней меньше людей, за 30 больше
- Новый статус `left` и порог `LEFT_STAFF_DAYS=21` — сотрудники без смены > 21 дня скрываются
- Каждая метрика API возвращает `{value, numerator, denominator, formula, unit}` — формулы на поверхности
- Новая секция `period_stats` в `/api/staff-list` — абсолюты (выручка/часы/ФОТ/смены) наверху UI
- Новая секция `contribution` в `/api/staff-detail` — доля сотрудника в часах/ФОТ/выручке ресторана
- Менеджеры считаются только за выбранный период (не по всей истории)
- UI «Период в цифрах» над KPI-карточками, формулы под каждым показателем

**2.9.1 — Реальные данные из ClickHouse:**
- Моки полностью удалены из `src/staff.ts`. Все 6 endpoints — SQL-запросы к **`db1`** (не `chicko`):
  - `db1.`ЧикоВремя`` (313K) — табель смен: Дата, Имя, Роль, Группа, РабВремяЧас, Начислено
  - `db1.`Чико4`` (183K) — дневной агрегат ресторана: ВыручкаБар/Кухня/Доставка, Начислено, Менеджер, ФудкостОбщий, Порча*, Питание/Мотивация/Проработка*
  - `db1.`ЧикоНов3`` (133K) — продажи по официантам: Дата, Официант, СреднийЧек, КолВоЧеков
- Таблица = один ресторан (только Калининград). `restaurant_id` валидируется, но не используется в WHERE
- Фильтр `left` в SQL: `HAVING toDate(factEnd) - MAX(Дата) <= 21`. Амоян/Долорет/Гусева автоматически исчезают из недавних периодов
- Классификация менеджеров динамическая — медианы `fot_pct`, `loss_pct`, `rev_per_day` считаются из данных периода
- Квантили bad/good shifts считаются из самих данных, не из хардкода
- Факт-only: `factEnd = min(end, today())` в Калининградской TZ (UTC+2). В `period.days_fact` возвращается количество фактических дней
- Корреляция Pearson(часы, выручка) считается в TypeScript по дневным парам

**2.9.3 — Admin Activity Analytics:**
- Новые endpoints `/api/admin/me` и `/api/admin/activity?window=7|30`
- Проверка прав через KV USERS поле `is_admin: true`. Non-admin → 403
- Вкладка «📊 Активность» в dashboard, видна только админам. Логика в `checkAdminAndShowTab()` — при 200 от `/api/admin/me` с `is_admin:true` вставляется `.ntab`-кнопка рядом с «Персонал» через `insertAdjacentElement('afterend', btn)`. Если не админ — вкладка физически не существует в DOM
- Детализация трекинга вкладок: `/api/activity` с action=tab теперь пишет endpoint как `/ui/tab/<name>` (overview/menu/staff/admin/...), не `/ui/tab`. Старые записи в SQL объединены как `/ui/tab (legacy)`
- Вкладка содержит: тулбар (7/30 дней + refresh), 5 KPI-карточек (DAU/WAU/событий/API-UI/ошибок с цветовой индикацией), sparkline DAU за 14 дней (сегодня зелёный), две колонки (топ UI-кликов + топ API endpoints с avg_ms и error-rate), таблица пользователей с индикатором давности (зелёная/жёлтая/серая точка)
- В KV USERS для `user:melnikov181076@gmail.com` добавлен флаг `is_admin: true`

**Итог session 9:** полный раздел персонала с реальными данными для владельца-франчайзи + внутренняя аналитика для контроля пилота. Паспорт пакта `STAFF_METRICS_PACT.md` обновлён v2 → v3 (добавлены разделы 18 «Статус left», 19 «Формулы на поверхности»).
...
## Следующие шаги — post Phase 2.9.3

**Приоритет 1 (активные баги, блокирующие UX):**
- **`/api/menu-analysis` 400 × 22 за 7 дней** — сам владелец замечает «страница меню часто не прогружается». Ошибки затрагивают 7 пользователей, включая `eda73ooo`, `kozhina39kaliningrad`, `anaselwilson`. Последняя 2026-04-24 11:44. Причина неясна, нужна диагностика через `wrangler tail`.
- **`/api/staff-*` 500 × 7 каждый** — сегодня утром 11:12 упали 5 endpoints: list, managers, groups, losses, performance. Затронули `melnikov181076`, `alfa_en`, `rucopyright`. Гипотеза: SQL заточен под Калининград (`db1.`Чико4``), другие рестораны не имеют данных и падают на null.
- **`/api/staff-*` 400 × 8 каждый** — тот же набор endpoints на другом witness-окне (11:31), похоже связано с тем, как валидатор обрабатывает отсутствие данных.

**Приоритет 2 (из pre-2.9 backlog):**
- **Re-enable AI кнопки в UI (Phase 2.8.3).** Backend `/api/ai-insight` готов, временно отключен для контроля расходов. 3 вызова за 7 дней у 1 пользователя — фича работает, но никто не знает.
- **Фикс пайплайна `dish_sales`.** Данные заканчиваются 2025-11-20, нужно обновление до 2026 г. Влияет на актуальность menu analysis и, возможно, связано с 400-ошибками выше.
- **Перенос домена `chicko.business-360.ru`.** Workers.dev блокируется РКН, франчайзи в РФ не открывается. План в session notes, deferred до стабилизации пилота.

**Backlog Phase 2.9+:**
- **Phase 2.9.4** — UI улучшения staff: scatter-plot для scatter_daily (сейчас только заглушка 0.72), AI Insight для персонала, фильтры в таблицах
- **Phase 2.10** — network-mode для staff (сейчас только single-restaurant)
- **Phase 2.11** — прогнозный модуль персонала («нужен Y-состав при выручке X»)

**Открытые вопросы пакта персонала v3 (не блокирующие):**
1. Роль «Франшиза менеджер» — класс оплаты?
2. Хосты → Management или Hall?
3. Fix_ЗП в ФОТ% — один показатель или два?

**Инфраструктурный backlog (из Phase 2.8):**
- Сетевая ротация меню 1 сентября — UI warning при выборе периода, захватывающего ротацию
...
- Рефакторинг: общие хелперы из `data_endpoints.ts`/`dow_profiles.ts`/`forecast.ts`/`menu_analysis.ts`

**Пилот:** ключевые разделы (Обзор, Динамика, Сравнение, Анализ, Меню, Персонал) готовы. Есть baseline-данные в Активности для отслеживания engagement. Следующий шаг — фикс 400/500 ошибок, чтобы новые франчайзи не спотыкались на первом входе.
...
**chicko.user_activity_log** (Phase 2.5, детализация в Phase 2.9.3):
- ts, user_id, email, endpoint, method, restaurant_id, response_status, response_ms, user_agent
- С Phase 2.9.3 события вкладок пишутся как `/ui/tab/<tab_name>` (не `/ui/tab`). Старые записи остаются, в SQL группируются как `/ui/tab (legacy)`.

**db1 (отдельная база на том же ClickHouse) — источник для Phase 2.9.1 Staff Analysis:**

`db1.`ЧикоВремя`` (313K строк) — табель смен Калининграда:
```
Дата          Nullable(Date32)
Имя           Nullable(String)      -- ключ, совпадает с ЧикоНов3.Официант
Роль          Nullable(String)
Группа        Nullable(String)       -- Кухня / Зал / Бар / Клининг / «-» (менеджмент)
РабВремяЧас   Nullable(Decimal(38,6))
Начислено     Nullable(Decimal(38,6))
Приход        Nullable(DateTime64(0))
```

`db1.`Чико4`` (183K строк) — дневной агрегат ресторана Калининграда:
```
Дата                Nullable(DateTime64(0))
Менеджер            String            -- менеджер дня, 'Отсутствовал' если не назначен
ВыручкаБар/Кухня/Доставка, СрЧек*, КолВоЧек*, СкидкаОбщий, ФудкостОбщий, ОценкаЯндекс, Оценка2Гис
Начислено, БарНачислено, ЗалНачислено, КлинингНачислено, КухняНачислено  -- ФОТ общий и по группам
ПорчаТовараБар, ПорчаТовараКухня, ПорчаВитрина, ПорчаПоВинеСотрудника, УдалениеБлюдСоСписанием, НедостачаИнвентаризации
ПитаниеПерсонала, МотивацияПерсонала, ПроработкаБар, ПроработкаКухня, ПроработкаБрендШеф, КлиентскийСервис, Представительские
  -- + около 30 других расходных статей (АктыСписания, СписанияИвент*, Дегустация*, УКФраншиза, и т.д.)
```

`db1.`ЧикоНов3`` (133K строк) — продажи по официантам Калининграда:
```
Дата               Nullable(DateTime64(0))
Официант           Nullable(String)        -- ключ = db1.ЧикоВремя.Имя
СреднийЧек         Nullable(Decimal(38,6))
КолВоЧеков         Nullable(Decimal(38,6))
СрКолВоПозВЧеке    Nullable(Decimal(38,6))
Скидка             Nullable(Decimal(38,6))
КассаСреднийЧек/КолВоЧеков/КолВоПозВЧеке
```

**Особенности `db1.*`:**
- Таблицы = один ресторан (только Калининград). Другие франчайзи пока не загружены.
- Русские имена колонок — в SQL обязательно в бэктиках: `` db1.`ЧикоВремя` ``
- Выручка официанта считается производно: `СреднийЧек × КолВоЧеков`. Явного поля выручки нет.
- Timezone в `Дата` принимается как UTC; в запросах приводится через `toDate()` для сравнения, при необходимости — `toTimeZone(Дата, 'Europe/Kaliningrad')`.
...
- **v3.37 (24.04.2026, день-9, день)** — Session 9 closed: Phase 2.9 цепочка (Staff Analysis skeleton → real ClickHouse → Admin Analytics). `src/staff.ts` переписан под `db1.*` таблицы с фильтром left (порог 21 день), факт/прогноз разделением, динамическими медианами для классификации менеджеров. Новая вкладка «📊 Активность» видна только админам (is_admin в KV USERS). `STAFF_METRICS_PACT.md` → v3. Обнаружены активные баги в menu-analysis и staff endpoints на не-Калининградских ресторанах — следующий приоритет.
