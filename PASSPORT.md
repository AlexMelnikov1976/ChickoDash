...
- `GET /api/staff-list?restaurant_id=N&start=…&end=…` — сводка персонала: численность, ФОТ%, KS-матрица официантов
- `GET /api/staff-detail?restaurant_id=N&employee_name=…&start=…&end=…` — детальная карточка сотрудника
- `GET /api/staff-groups?restaurant_id=N&start=…&end=…` — ежедневная динамика выручки и ФОТ
- `GET /api/staff-performance?restaurant_id=N&start=…&end=…` — перформанс официантов: выручка, чеки, часы
- `GET /api/staff-managers?restaurant_id=N&start=…&end=…` — менеджеры смены
- `GET /api/staff-losses?restaurant_id=N&start=…&end=…` — потери категорий A/B/C
- `GET /api/grill-daily?restaurant_id=N&start=…&end=…` — ежедневная выручка по гриль-группам (Phase 2.9.4)
- `GET /api/owner/me` — пробник прав владельца (`is_owner`); 200 всегда (Phase 2.12)
- `GET /api/owner/history` — daily-факт по Калининграду (dept_id=101) с 2024-05-01, для Owner P&L (Phase 2.12)
- `GET /api/owner/costs` — постоянные затраты и нормативы владельца (KV или дефолты УпрЧико) (Phase 2.12)
- `POST /api/owner/costs` — сохранить редактируемые постоянные затраты в KV `owner:costs:<email>` (Phase 2.12)
...
- **Fix D:** Сводная таблица «Показатели ресторанов за период» на вкладке Обзор:
  ...
  - Колонка «Глубина» (позиций в чеке) — **реализована в Phase 2.11**, 7 колонок в таблице
...
- Вкладка Персонал была помечена `locked` + lock-badge 🔒, контент «Скоро / Q2 2026» (снято в Phase 2.11)
...
### Phase 2.10.7 — Marketing tooltips ✅ ГОТОВ (25.04.2026)

Hover-подсказки на 46 плашек вкладки «💎 Маркетинг». Цель — франчайзи видит цифру, наводит мышь, понимает что это, как считается и как читать тренд. До этого только заголовок плашки + цифра, без методики — пользователь либо доверяет вслепую, либо игнорирует.

**Что добавлено в `public/`:**

- `marketing-tips.js` — словарь `MKT_TIPS` (47 ключей × 3 секции: Что / Как считается / Как читать) + ванилла-JS engine (~80 строк) с event delegation на document, корректировкой позиции под viewport и скрытием на скролле
- `marketing-tips.css` — изолированные стили под `#mkt-tooltip` (`@media (hover: none)` скрывает на мобильном, согласовано — там hover нет; `cursor: help` на `[data-mkt-tip]` как сигнал интерактива)
- `dashboard.html` — два подключения: `<link>` на CSS, `<script defer>` на JS
- `dashboard.js` — `mktDraw` получил 25 атрибутов `data-mkt-tip` (на 5 KPI, 6 шагов воронки + утечку, 4 обёртки графиков, балансы, LTV, health, аномалии). Пять хелперов (`mktSegRow`, `mktCampCard`, `mktBalRow`, `mktLtvRow`, `mktHealthRow`) расширены опциональным последним параметром `tipKey` — обратная совместимость сохранена, в проекте они нигде кроме `mktDraw` не вызываются.

**Технические решения:**

- Hover на всю плашку (не иконка ⓘ) — выбор сделан осознанно: меньше визуального шума, плашки и так маленькие. Минус — на мобильном бесполезно, но франчайзи на телефоне смотрит цифры, не разбирает методику.
- Event delegation на `document.mouseover/mouseout` — работает для динамической перерисовки `mktDraw` при смене периода (7д/30д/90д/...), не нужно перевешивать listeners.
- Позиционирование с защитой от viewport: tooltip пробует появиться под плашкой по центру, при отсутствии места снизу — наверх; уезжает за правый край — сдвигается влево; никогда не вылезает за экран. Скролл закрывает (чтобы не «зависал» в воздухе).
- TS не трогали — это чистый front-end ассет, `npx tsc --noEmit` не нужен.

**Итерации:**

*v1 — первый деплой.* Тексты подсказок с SQL-формулами в духе `SUM(bal_total_sum) = SUM(bal_gift + bal_accumulated + bal_promo)` или `quantile(0.50)(revenue_total)`. После деплоя Алекс прислал скриншот с замечанием: «формулу читать сложно и не понять что означают переменные». Справедливо — это формат для разработчика, не для франчайзи.

*v2 (актуальная)* — все 47 текстов переписаны человеческим языком:
- `SUM(bal_gift + bal_accumulated + bal_promo)` → «складываем три типа бонусов: подарочные (welcome), накопленный кэшбэк и промо-бонусы»
- `recency_days BETWEEN 90 AND 180` → «не были 3-6 месяцев»
- `COUNT(DISTINCT phone)` → «считаем уникальные телефоны»
- `quantile(0.50)` → «выстраиваем всех клиентов по сумме покупок и берём того, кто оказался ровно в середине»

Секция в шапке тултипа переименована «Формула:» → «Как считается:» — обязывает писать словами. Контрольная проверка автоматическая: `grep -c "SUM\|COUNT\|AVG\|quantile" marketing-tips.js` → 0 в v2 (было 28+ в v1).

**Покрытие:** 46 плашек из 47 ключей словаря покрыты `data-mkt-tip`. Единственный неиспользуемый ключ — `loyalty.other` (этой группы в текущем UI нет, оставлен в словаре на случай появления).

**Файлы и размеры:**
```
public/marketing-tips.js   ~32 КБ (словарь + engine)
public/marketing-tips.css  ~1.8 КБ
public/dashboard.html      +2 строки (подключения)
public/dashboard.js        +1.5 КБ (data-атрибуты + 6-й аргумент в хелперах)
```

**Редактирование текстов** — без перезагрузки сервера, пересборки Worker не требует. Меняем `MKT_TIPS` в `public/marketing-tips.js` → `npx wrangler deploy` подцепит как статику.

### Phase 2.11 — Personnel Tab Launch ✅ ГОТОВ (27.04.2026)

Запуск вкладки «👥 Персонал» в продакшне. Вкладка доступна только для Калининграда (источник данных — db1.ЧикоВремя, db1.ЧикоНов3, db1.Чико4 привязаны к одной точке). На других ресторанах — graceful fallback.

**Что исправлено:**

**Баг A — Revenue mismatch (387к ₽ vs 286к ₽):**
- Root cause: `handleStaffList` и `handleStaffGroups` брали выручку из `db1.Чико4`, которая включает доходы от агрегаторов доставки не так, как основной пайплайн, и не фильтрует по `restaurant_id`. Результат: другая цифра выручки, чем на вкладке Обзор.
- Fix: переключены на `chicko.mart_restaurant_daily_base WHERE dept_id = v.restId` — тот же источник что у Обзора. Начисления ФОТ оставлены в `db1.Чико4` отдельным запросом `sqlPayroll`.

**Баг B — KS-матрица пустая (`total_productive_employees: 0`):**
- Root cause: ClickHouse в JOIN-запросах без явных `AS`-алиасов возвращает колонки с prefix-ом таблицы: `"n.employee_name"`, `"wg.last_shift_global"` и т.д. Код обращался к `p.last_shift_global` (undefined) — все строки отфильтровывались по 21-дневному порогу.
- Fix: добавлены явные `AS employee_name`, `AS role_primary`, `AS hours_total`, `AS last_shift_global` во все SELECT в `sqlPerf`.

**Баг C — Часы = 0 для последних дней:**
- Root cause: `db1.ЧикоВремя` (данные из iiko) обновляется с задержкой ~7 дней. Для периодов ближе апреля 20+ часы реально отсутствуют в источнике.
- Fix: добавлено поле `workdata_through` в meta-ответ API — дата последней записи в ЧикоВремя. В UI `renderStaffPayrollWarning` показывает предупреждение если `end > workdata_through`.

**UI:**
- Вкладка «👥 Персонал» разблокирована (убран `locked` класс и lock-badge 🔒)
- Добавлен баннер «Данные по Калининграду» — калининградский ресторан
- Предупреждение о лаге часовых данных в шапке раздела

**Известные ограничения:**
- `staff-managers`, `staff-losses`, `staff-detail` всё ещё используют `db1.Чико4` для соотношений ФОТ% (не критично — ФОТ% изменился незначительно, правим в следующей итерации)
- Вкладка не работает для других франчайзи (Phase 2.9.5 — нормализация таблиц)
- В ЧикоНов3 присутствует тестовая запись «aqba» / «Кожина Виктория Геннадьевна» (роль: Системный администратор) — попадает в KS-матрицу

### Phase 2.11.1 — Staff Ratings + Tooltips ✅ ГОТОВ (27.04.2026)

Рейтинг сотрудников 0–100 по группам на основе гостевых оценок (Звезд_1..5 из Чико4) и операционных метрик.

**Логика рейтинга:**

- **Зал (кассиры/официанты):** Гостевой балл 40 + Выручка/час vs медиана 35 + Средний чек vs медиана 25
- **Менеджмент:** Гостевой балл 35 + Доля сильных смен 30 + Контроль потерь 20 + Доля гриля 15 (цель 3%)
- **Кухня/Бар/Клининг:** Гостевой балл 60 + Явка/часы vs медиана 40

**Принцип «коллективной ответственности»:** у гостей нет атрибуции на конкретного сотрудника. Гостевой балл дня распределяется равномерно между всеми, кто вышел в смену в этот день.

**Бэкенд:** `GET /api/staff-ratings` → `handleStaffRatings` в `src/staff.ts`. Четыре параллельных запроса: ЧикоВремя (смены) + ЧикоНов3 (продажи) + Чико4 (оценки + выручка + потери) + chicko.dish_sales (гриль). Возвращает `ratings[]`, `bad_days[]`, `summary`.

**Фронтенд:** `renderStaffRatings(data)` в `dashboard.js` — три групповые таблицы + блок «Дни с плохими оценками» (day_score < 3★). Бейджи компонентов с числовыми баллами.

**Hover-подсказки (staff-tips):**
- `public/staff-tips.js` — 12 ключей (`rating.group.*`, `rating.comp.*`, `rating.bad_days`, `rating.summary`)
- `public/staff-tips.css` — стили, изолированы под `#staff-tooltip`
- Атрибут `data-staff-tip="..."` на заголовках групп, бейджах компонентов, строке источника данных
- Тот же engine (event delegation), что и `marketing-tips.js`

### Phase 2.12 — Owner P&L ✅ ГОТОВ (28.04.2026)

Вкладка «👑 Owner PnL» в основном дашборде (`dashboard.html`) — план/факт/прогноз по Калининграду, сценарные слайдеры, P&L-таблица, водопад, драйверы. Видна только владельцу; команда (27 юзеров) вкладку не замечает.

**Доступ:**
- ~~Флаг `is_owner: true`~~ — **снято (01.05.2026)**: вкладка видна всем авторизованным пользователям. `checkOwnerAndShowTab()` больше не вызывает `/api/owner/me`.
- `/api/owner/*` по-прежнему работают только с данными Калининграда (dept_id=101, захардкожен).

**Бэкенд (`src/owner.ts`):**
- `GET /api/owner/me` — probe, 200 всегда.
- `GET /api/owner/history` — daily-факт по **dept_id=42** (Калининград) из `chicko.mart_restaurant_daily_base` + месячный ФОТ персонала из `db1.ЧикоВремя`. Окно с 2024-05-01. Возвращает `{data[], monthly[], sliderDefaults, mgtSalary}`.
  - `sliderDefaults.fotPct` = полный ФОТ % (персонал + управляющий 657 500) последнего полного месяца.
- `GET /api/owner/costs` / `POST /api/owner/costs` — нормативы в KV `owner:costs:<email>`. Дефолты из УпрЧико: rent 650k, util 50k, mgtSalary 657.5k, salaryTax 16%, foodcost 22.5%, franchise 5%, writeoff 2.3%, hozy 4%, acquiring 1.9%, deliveryCost 31.44%, usn 15%.

**Фронтенд (namespace `ownerPnl_*`, `OP_*` в `dashboard.js`):**

- **Факт:** суммирует дни ≤ вчера включительно (timezone-safe `localISO`). Совпадает с цифрой вкладки Обзор.
- **Прогноз:** DOW-медианы текущего месяца (Method A, ≥7 дней) → если меньше, fallback на `autoPlanDay`. Идентичен алгоритму вкладки Обзор. При дефолтных слайдерах прогноз совпадает с Обзором.
- **Слайдеры:** инициализируются из реальных медиан текущего месяца (`ownerPnl_computeSliderDefaults`): чеки будни/выходные, ср. чек, скидки.
- **Чарт:** 4 серии — Норм/план (серый), Факт (зелёный), Прогноз (синий), Сценарий (штрихованный, фиолетовый).
- **P&L calcPnL:** `staffTotal = revenue × fotPct%`; mgtSalary (657 500) уже включён в fotPct — не добавляется повторно. `fotPersonnel = staffTotal - mgtSalary`.
- **Нормативы** `OP_NORMS` из УпрЧико: foodcost 22.5%, franchise 5%, writeoff 2.3%, hozy 4%, acquiring 1.9%, deliveryCost 31.44%.
- Тултип на плашке «📅 Факт + прогноз месяца».

**Маршрутизация (`src/index.ts`):** 4 роута `/api/owner/*` аддитивно. Роут `/owner-pnl` сохранён для совместимости (редирект или 404).

**KV-ключи:**
- `user:<email>` — `is_owner: true`
- `owner:costs:<email>` — пользовательские нормативы

**Изоляция:** `src/owner.ts` не трогает другие хендлеры. Все изменения в `dashboard.js` в namespace `ownerPnl_*`/`OP_*` — не пересекаются с другими вкладками.

**Известные ограничения / TODO:**
- Убрать `public/owner-pnl.html` и роут `/owner-pnl` из `src/index.ts` (устаревшие артефакты).
- 💼 PnL таб (обычный, для команды) скрыт (`style="display:none"` в nav) — вернуть когда потребуется.
...
### Phase 2.13 — UX Polish ✅ ГОТОВ (01.05.2026)

Серия точечных UX-улучшений поверх Phase 2.12.

**City lock на Staff + Owner PnL (`goTab()` в dashboard.js):**
- При входе на вкладки `staff` / `owner-pnl` сохраняется `{r, restIdx, netMode}` в `_KALN_SAVED`, R переключается на Калининград, `_KALN_LOCKED = true`.
- В дропдауне все не-калининградские рестораны: `opacity:0.3`, `pointer-events:none` — видно что они есть, но нельзя выбрать.
- Переключатель «Вся сеть» приглушён (`opacity:0.35`, `pointer-events:none`).
- При выходе с Kaln-вкладки на любую другую — полное восстановление R, NETWORK_MODE, UI.
- Переход `staff ↔ owner-pnl` — без изменений (состояние уже заморожено).

**Owner PnL для всех:** `checkOwnerAndShowTab()` синхронная, без проверки `is_owner`. Вкладка видна всем.

**Меню — чип «📋 Все»:**
- `renderClassesBreakdown()`: новый чип первым в гриде (9 колонок вместо 8 в CSS).
- Клик → `clearMenuClassFilter()`: снимает фильтр класса, показывает все блюда с колонкой «Класс».
- Активное состояние (gold border) синхронизировано с `setMenuActiveClass()`.
- Плашка «Всего блюд» в KPI-ряду тоже кликабельна — делает то же самое.

**Обзор — формат выручки:** `fmtR(v)` → `fmtR(v, true)` в колонке «Выручка» таблицы ресторанов. Теперь `1 598 909 ₽` вместо `1.6М₽`.

**Маркетинг — «было N» в дельтах KPI:**
- `mktUpdateDelta` и `mktUpdateDeltaPct` получили опциональный `prevVal`.
- Дельта теперь: `▲ +234 за 30д · было 23 216` (было приглушено opacity 0.6).
- Работает для всех 4 карточек: База / Активны / Repeat / LTV. Пересчитывается при смене периода.

**Фиксы (1 мая, первый день месяца):**
- `ownerPnl_computeSliderDefaults()` и `ownerPnl_computeFactMetrics()`: `new Date()` → `new Date(OP_TODAY)`. Слайдеры и факт-метрики инициализировались пустыми если ClickHouse ещё не залил данные за текущий месяц.
- `renderPnl()`: убрали `pnlEnsureKaliningrad()` — блокировал city switcher на основном P&L табе.

**Прогноз по выбранному периоду (`as_of`):**
- `fetchForecast()` (клиент): передаёт `as_of=CAL_STATE.global.end` в запрос. Кэш разделён по ключу `restId|as_of` — при смене периода перезапрашивает.
- `handleForecast()` (сервер `src/forecast.ts`): читает `as_of` из query params, валидирует формат `YYYY-MM-DD` и что дата не в будущем. Использует как опорную дату вместо жёсткого `today()`. Fallback → сегодня.
- Результат: при выборе любого прошлого месяца прогноз пересчитывается на этот месяц (Method A — медианы самого месяца, B — прошлый год × YoY, C — DOW-профиль 90 дней).

**Локальный preview (dev only):** `src/data_endpoints.ts` + `src/security.ts` — мок-данные 4 ресторанов и auth bypass для `localhost`. В prod не влияют (hostname никогда не будет localhost).

## Следующие шаги — post Phase 2.13
...
**Приоритет 2:** Фикс staff-* для мульти-франчайзи (Phase 2.9.5). Вкладка Персонал сейчас работает только для Калининграда. Таблицы `db1.ЧикоВремя`, `db1.Чико4`, `db1.ЧикоНов3` привязаны к одному ресторану. На других франчайзи endpoints возвращают 500/пустые данные. Правильный фикс: нормализация таблиц с `dept_id`/`dept_uuid`. Быстрый фикс: detect по `city !== 'Калининград'` → показать заглушку «Данные персонала доступны только для Калининграда».

**Приоритет 3:** Убрать устаревший `public/owner-pnl.html` и роут `/owner-pnl` из `src/index.ts`.
...
- ~~Добавить `avg_items_per_check` в `mart_restaurant_daily_base` + пайплайн n8n → колонка «Глубина» в сводной таблице~~ → **выполнено в Phase 2.11** (JOIN с `daily_extract`)
...
Прод: 64 ресторана, 1 515 уникальных dish_code, период с 2024-01-02. Пайплайн обновляется ежедневно с лагом 1 день (актуальность D-1). Снимок 22.04.2026: 2 584 636 строк до 2025-11-20 — устаревшая статистика, текущие объёмы выше; для актуального счётчика читать `SELECT count(), max(report_date) FROM chicko.dish_sales`.

**chicko.dish_cooking_daily** — дневные cooking-метрики по блюдам (источник для анализа кухни и SLA):

```
report_date                 Date
dept_uuid                   String                 -- iiko Department.Id, JOIN-ключ к dish_sales
restaurant_name             String                 -- iiko Department (короткое имя торгового предприятия)
city                        LowCardinality(String) -- из dim_restaurants; для не покрытых dept_uuid пусто
dish_code                   String                 -- стабильный SKU
dish_name                   String
dish_category               String
dish_group                  String
cooking_duration_avg_sec    Nullable(UInt32)       -- Cooking.CookingDuration.Avg
kitchen_time_avg_sec        Nullable(UInt32)       -- Cooking.KitchenTime.Avg
guest_wait_avg_sec          Nullable(UInt32)       -- Cooking.GuestWaitTime.Avg
serve_time_avg_sec          Nullable(UInt32)       -- Cooking.ServeTime.Avg
start_delay_avg_sec         Nullable(UInt32)       -- Cooking.StartDelayTime.Avg
cooking_late_avg_sec        Nullable(UInt32)       -- Cooking.CookingLateTime.Avg
feed_late_avg_sec           Nullable(UInt32)       -- Cooking.FeedLateTime.Avg
qty_total                   Float64                -- DishAmountInt из OLAP (без фильтров на удаления)
has_cooking_data            UInt8                  -- 1 если ≥1 cooking_*_avg_sec не NULL
inserted_at                 DateTime DEFAULT now()
source_system               LowCardinality(String) DEFAULT 'iiko_olap'
```

ENGINE: `ReplacingMergeTree(inserted_at)`, PARTITION BY `toYYYYMM(report_date)`, ORDER BY `(report_date, dept_uuid, dish_code, dish_name)`.

Источник: iiko Server OLAP `/resto/api/v2/reports/olap` (reportType=SALES, агрегаты `Cooking.*.Avg` в секундах). Один OLAP-вызов на день, группировка `Department.Id × Department × DishCode × DishName × DishCategory × DishGroup`.

Стартовый бэкфил 08.05.2026: период 2026-01-01 .. 2026-05-07 (127 дней), 772 754 строки, 50 ресторанов, 2 266 уникальных dish_code. Покрытие cooking-данными: 96.9% (3.1% строк — модификаторы, доставка-комплекты, чай навынос — у них нет сервисной печати). Скрипт: `infra/iiko/backfill_cooking.py`. Регулярная загрузка через n8n — TODO (пока не настроена).

Особенности:
- `qty_total` ≠ `dish_sales.qty` для одного и того же `(report_date, dept_uuid, dish_code)`: в моём OLAP-запросе нет фильтра `DeletedWithWriteoff`, тогда как `dish_sales` исключает удалённые позиции. Расхождение обычно 1–5%. Для cooking-метрик это намеренно (нужна реальная нагрузка на кухню, включая отменённые после готовки заказы).
- `restaurant_name` отличается от `dish_sales.restaurant_name` — в OLAP короткое имя ТП (`МСК Покровка`), в `dish_sales` полное юр.имя. Связка через `dept_uuid` единственный надёжный ключ.
- `city` пуст для 24 ресторанов — их нет в `chicko.dim_restaurants` (последняя содержит только 26 точек, тогда как сеть фактически 50+). Это ограничение `dim_restaurants`, не cooking-загрузки.

JOIN с `dish_sales` для совмещения продаж и времени готовки:
```sql
SELECT s.restaurant_name, s.dish_name, s.qty, s.revenue_rub,
       c.cooking_duration_avg_sec, c.kitchen_time_avg_sec, c.guest_wait_avg_sec
FROM chicko.dish_sales s FINAL
LEFT JOIN chicko.dish_cooking_daily c FINAL
  USING (report_date, dept_uuid, dish_code)
...
**security.ts:**
- `MAX_DATE_RANGE_DAYS = 550` — расширено с 400 до 550 (Phase 2.11) для поддержки диапазона с 01.01.2025
...
- **v3.47 (08.05.2026)** — Блюд/чек на вкладках Динамика, Сравнение, Анализ. Динамика: карточка с графиком «Блюд/чек» (скрыта при отсутствии данных), кнопка «Блюд/чек» в DOW-переключателе, строка в таблице статистики периода. Сравнение: кнопка «Блюд/чек» в метриках, строка в детальной таблице, строка в таблице vs Сеть/ТОП-10. Анализ: строки «Блюд/чек» в блоках будни/выходные, слайдеры `sl-wd-ipc`/`sl-we-ipc` (показываются только при наличии данных в 90д baseline), мультипликативный эффект на средний чек (`effective_chk = wdChk × ipc/baseline_ipc`), шаги «Будни/Выходные — блюд/чек» в waterfall. `computePlContext` добавлены `wdBase.ipc`/`weBase.ipc`.
- **v3.46 (07.05.2026)** — Глубина чека (avg_items_per_check) на вкладке Обзор. Backend: LEFT JOIN `chicko.daily_extract FINAL` в `/api/restaurants` (поле `avg_items_per_check_total`) и `/api/benchmarks` (`quantileIf` 0.50/0.75 → `net.itemsPerCheck` / `top10.itemsPerCheck`). Frontend: плашка KPI «Блюд / чек» (скрывается если данных нет), 7-я колонка «Блюд/чек» в сводной таблице ресторанов, алерт при глубине <85% медианы сети, позитивный алерт при топ-25%, компонент «Глубина» в разбивке Score (вес 5%, снято с фудкоста 0.25→0.20). Aggregation в buildNetworkR и groupTs — weighted avg по чекам. Данные берутся из `daily_extract`, не требуют изменений пайплайна n8n.
- **v3.45 (28.04.2026)** — Phase 2.12 closed: Owner P&L tab задеплоена. dept_id=42 → dept_id=101 (правильный ID Калининграда). Слайдеры инициализированы из реальных медиан CH. P&L таблица, водопад, драйверы.
- **v3.43 (27.04.2026)** — Phase 2.11.1 closed: рейтинг сотрудников 0-100 (3 группы: Зал/Менеджмент/Другие). Гостевой балл через Звезд_1..5 из Чико4 (коллективная ответственность за день). Менеджерам добавлена доля гриля (цель 3%). КлиентскийСервис исключён. Новый endpoint GET /api/staff-ratings. Hover-подсказки staff-tips.{js,css} — 12 ключей, тот же engine что marketing-tips. Атрибуты data-staff-tip на заголовках групп, бейджах компонентов, строке источника.
- **v3.42 (27.04.2026)** — Phase 2.11 closed: запуск вкладки Персонал. Revenue source fix: Чико4 → mart_restaurant_daily_base (устранён mismatch 387к vs 286к). KS-матрица: явные AS-алиасы во всех SELECT в sqlPerf (ClickHouse возвращал префиксные ключи "n.employee_name" вместо "employee_name" — все строки отфильтровывались). workdata_through в meta API + предупреждение о лаге часов ~7 дней в UI. /api/grill-daily endpoint (Phase 2.9.4). authFromCookieOrServiceKey для n8n service key. MAX_DATE_RANGE_DAYS расширен 400→550. Вкладка разблокирована, добавлен Калининград-баннер.
- **v3.41 (25.04.2026)** — Phase 2.10.7 closed: hover-подсказки на 46 плашек вкладки Маркетинг (KPI, воронка, RFM, лояльность, кампании, балансы, LTV, health, графики). Новый модуль `public/marketing-tips.{js,css}` с словарём `MKT_TIPS` (47 ключей × 3 секции — Что / Как считается / Как читать) + ванилла-JS engine с event delegation. В `mktDraw` добавлены 25 атрибутов `data-mkt-tip`, 5 хелперов получили опциональный `tipKey`. v1 деплой провалился по UX (формулы в SQL-стиле непонятны франчайзи) → v2 переписан человеческим языком, секция «Формула» → «Как считается», контрольная проверка `grep SUM|COUNT|quantile` = 0. Hover на всю плашку, на мобильном tooltip скрыт через `@media (hover: none)`. Покрытие 46/47, единственный неиспользуемый ключ — `loyalty.other` (этой группы в UI нет).
