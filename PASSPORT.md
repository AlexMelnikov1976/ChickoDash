...
**Focus:** Phase 2.12 — Owner P&L (изолированный раздел владельца, реальные данные ClickHouse, столбчатый план/факт/прогноз)
...
### Phase 2.12 — Owner P&L (изолированный раздел владельца) ✅ ГОТОВ (28.04.2026)

Отдельный раздел для владельца сети — план/факт/прогноз по Калининграду, сценарные слайдеры, P&L-таблица, водопад, драйверы. Изолирован от основного дашборда: команда (27 юзеров) видит только свой dashboard, владелец дополнительно — `/owner-pnl`.

**Доступ:**
- Флаг `is_owner: true` в KV USERS (`user:<email>`). Сейчас включён только для `melnikov181076@gmail.com`. Остальные юзеры на `/api/owner/*` получают 403.
- HTML `/owner-pnl` открыт всем (за стеной cookie-auth основного дашборда), но внутри сразу зовёт `/api/owner/me` и при `is_owner !== true` показывает «Раздел доступен только владельцу».

**Бэкенд (`src/owner.ts` — новый файл, ничего не правит в других хендлерах):**
- `GET /api/owner/me` — лёгкий probe для фронта, 200 всегда (`{is_owner, email}`).
- `GET /api/owner/history` — daily-факт по dept_id=101 (Калининград) из `chicko.mart_restaurant_daily_base`, поля: revenue, revenueDelivery, checks, avgCheck, foodcostPct, discPct, deliverySharePct, isWe. Окно с 2024-05-01.
- `GET /api/owner/costs` — постоянные/нормативы. Возвращает `{costs, defaults}`. Дефолты из УпрЧико: rent 200k, util 50k, штат 250k, упр 120k, fc 32%, фрш 4%, спис 1.5%, хоз 1%, экв 1.5%, доставка 15%/33%, налог ФОТ 16%.
- `POST /api/owner/costs` — сохранить в KV `owner:costs:<email>` (Origin-check + санитизация: только числа в [0..1e9]).

**Фронтенд (`public/owner-pnl.html`, копия локального макета):**
- Async-bootstrap: проверка `is_owner` → параллельные `fetch('/api/owner/history')` + `fetch('/api/owner/costs')` → `HISTORY/TODAY/NORMS` живут как `let` и заполняются после ответа, `initDashboard()` вызывается только после успешной загрузки.
- PIN-overlay переиспользован как «Загрузка / Доступ запрещён» — сами PIN-кнопки оставлены в DOM, но не активны (auth теперь чисто серверная).
- `window.saveOwnerCosts(patch)` — сохранение редактированных постоянных в KV (для будущего UI редактирования).

**Диаграмма «Выручка по периодам — -3 / +6»:**
- Группировка по 4 режимам (week/month/quarter/year) через `getChartBuckets(mode)`. Окно: 3 периода в прошлом + текущий + 6 в будущем (всего 10 столбцов на оси X).
- Тип — столбчатая, 3 столбика рядом на каждый период: оранжевый (План), зелёный (Факт), синий (Прогноз). Над каждым столбиком подпись значения (`fmtS` — К/М). Маркер «сейчас» — пунктирная вертикаль с подписью на текущем периоде.

**Маршрутизация (`src/index.ts` — добавлено только аддитивно):**
- `GET /owner-pnl` → `env.ASSETS.fetch('/owner-pnl.html')` + те же security headers, что и у основного дашборда.
- 4 роута `/api/owner/*` после блока Marketing.

**KV-ключи раздела:**
- `user:<email>` — JSON с флагом `is_owner: true` (если задан).
- `owner:costs:<email>` — пользовательские постоянные затраты, перекрывают дефолты.

**Изоляция:** только новый файл `src/owner.ts`, новый ассет `public/owner-pnl.html`, аддитивные роуты в `src/index.ts`. `dashboard.{html,css,js}` и существующие хендлеры (overview/dynamics/compare/analysis/menu/staff/marketing/admin) не тронуты.
...
## Следующие шаги — post Phase 2.11
