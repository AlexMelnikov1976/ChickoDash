# Chicko Analytics — Раздел «Персонал», пакт метрик v3

**Статус:** v3 утверждён 2026-04-23, Phase 2.9.2b (session 9.3)
**Предыдущая версия:** v2 (session 9.2) — дополнена разделами 18 (статус `left`) и 19 (формулы в ответах API)
**Зависимости:** `mart_restaurant_daily_base.is_anomaly_day`, источник iikoWeb через n8n

## Что нового в v3

Относительно v2:
- **+ Раздел 18 «Статус `left` и порог LEFT_STAFF_DAYS»** — формализация понятия «уволенный» (last_shift > 21 дня), исключение из активного штата
- **+ Раздел 19 «Формулы на поверхности»** — принцип: каждая метрика в ответе API возвращается как `{value, numerator, denominator, formula, unit}`, UI показывает формулу под каждой цифрой
- **+ Секция `period_stats`** в ответе `/api/staff-list` — абсолютные значения (выручка, часы, ФОТ, смены) для визуального контекста
- **+ Секция `contribution`** в `/api/staff-detail` — вклад сотрудника в часы/ФОТ/выручку ресторана
- **+ Менеджеры считаются только за выбранный период** — `days_as_manager` пропорционален периоду, не всей истории
- Моки в `staff.ts` теперь детерминированно зависят от `daysBetween(start, end)`
...
## 18. Статус `left` — уволенные сотрудники

### 18.1 Проблема v2

В v1/v2 статус сотрудника определялся 4 категориями: `new`, `core`, `regular`, `occasional`, `dormant`. Но `dormant` (14-21+ дней без смены) размывал две совершенно разные ситуации:
- Сотрудник в отпуске/болезни, который вернётся — **включаем в анализ**
- Сотрудник, который фактически уволен, но формально не отмечен таковым в iiko — **анализ бесполезен, искажает метрики**

Жалоба франчайзи после Phase 2.9.2 подтвердила: «сотрудники, которые не работают больше 3-х недель, нет смысла анализировать, они уволены».

### 18.2 Решение v3

Вводится новый порог и статус:

```
LEFT_STAFF_DAYS = 21
```

Обновлённая классификация статусов:

| Статус | Условие | В KPI | В таблице |
|---|---|---|---|
| `new` | `first_shift_ever` в последние 30 дней до `end` | ✅ | ✅ |
| `core` | `hours_total` ≥ P75 по группе в периоде | ✅ | ✅ |
| `regular` | не попал в core/new/occasional/dormant/left | ✅ | ✅ |
| `occasional` | `shifts_count / days_in_period < 0.3` и `tenure >= 60` | ✅ | ✅ |
| `dormant` | `14 < days_since_last_shift <= 21` и `tenure >= 60` | ✅ с пометкой | ✅ с иконкой 💤 |
| **`left`** 🆕 | `days_since_last_shift > 21` | ❌ **исключаем** | ❌ **скрываем** (опция «Показать уволенных») |

### 18.3 Применение в SQL

Базовый фильтр во всех запросах API Staff:

```sql
WHERE employee_id NOT IN (
  SELECT employee_id
  FROM chicko.staff_shifts
  WHERE dept_uuid = @dept_uuid
  GROUP BY employee_id
  HAVING dateDiff('day', MAX(report_date), @end) > 21
)
```

Это исключает `left`-сотрудников **из всех** агрегатов: `active_headcount`, `total_hours`, `total_payroll`, breakdown групп, KS-матрицы, ротации.

### 18.4 UI

- В основной таблице сотрудников показываем только статусы `new / core / regular / occasional / dormant`
- `left`-сотрудники скрыты по умолчанию
- Счётчик `excluded_left_count` в summary показывает, сколько отфильтровано: «Исключено 3 уволенных (> 21 дня без смены)»
- Опция **«Показать уволенных»** (чекбокс в тулбаре таблицы) — Phase 2.9.2c (не делаем сейчас)

### 18.5 Ротация

Ротация в v3 считается с учётом `left`:

```
rotation_pct = (новых в периоде + уволившихся в периоде) / активный_состав × 100
```

где **«уволившихся в периоде»** = сотрудники, которые в течение `[start..end]` имели свой последний `last_shift` и после него не появлялись ≥ 21 дня на момент `end`.

В моке Phase 2.9.2b пока считаем только новых (уволившихся в моке нет).

---

## 19. Формулы на поверхности

### 19.1 Проблема v2

Ответ API возвращал плоские числа: `{payroll_pct_of_revenue: 13.0}`. Пользователь видел «13%» и не понимал:
- 13% от **какой** выручки? За какой период?
- Какой абсолютный ФОТ?
- Как связано с выручкой, которую он видит на других вкладках?

В моках Phase 2.9.2 это было незаметно, потому что все цифры выглядели «ок». Но как только появятся настоящие данные, пользователь **сможет не поверить** цифре. Без возможности проверки формула = бесполезна.

### 19.2 Принцип

**Каждая производная метрика в ответе API возвращается в виде объекта:**

```typescript
{
  value: number,              // собственно число
  numerator?: number,         // абсолютный числитель
  denominator?: number,       // абсолютный знаменатель
  formula: string,            // человекочитаемое описание
  unit: '%' | '₽' | '₽/час' | 'чел' | 'дн' | string,
  norm?: { min: number, max: number }, // опционально — нормальный диапазон
}
```

Пример (ответ `/api/staff-list`):
...
### 19.3 Применение в UI

Под каждой KPI-карточкой выводится сжатая формула:

```
┌─────────────────────────┐
│ ФОТ %                   │
│ 13.0%                   │  ← value
│ 1.58M ₽ / 12.1M ₽       │  ← absoluts from num/den
└─────────────────────────┘
```

Для простых метрик (без числителя/знаменателя) — отображается строка `formula`.

### 19.4 Секция `period_stats` в ответе `/api/staff-list`

Новая секция поверх всего ответа:

```json
"period_stats": {
  "days_in_period": 51,
  "revenue_total_rub": 19415700,
  "revenue_per_day_avg_rub": 380700,
  "hours_total": 4290,
  "shifts_total": 310,
  "payroll_total_rub": 1578340
}
```

Эти числа рендерятся в первом блоке UI — **«Период в цифрах»** — до любых KPI. Пользователь сначала видит абсолютные значения ресторана за период, потом уже производные проценты.

### 19.5 Секция `contribution` в ответе `/api/staff-detail`

В drawer отдельного сотрудника:

```json
"contribution": {
  "hours_share_pct": 5.2,
  "hours_share_formula": "196 ч ÷ 4290 ч ресторана × 100",
  "payroll_share_pct": 3.1,
  "payroll_share_formula": "45 000 ₽ ÷ 1 578 340 ₽ ФОТ × 100",
  "revenue_attributed_rub": 2 337 300,
  "revenue_share_pct": 12.0,
  "revenue_share_formula": "2 337 300 ₽ его выручки ÷ 19 415 700 ₽ ресторана × 100"
}
```

Пользователь сразу видит не только собственные цифры сотрудника, но и его **долю** в ресторане.

### 19.6 Формулы в таблицах — tooltip'ы

Для метрик внутри таблиц (менеджеры, группы, потери) формулы отображаются в `title=""` атрибуте — виден при hover'е. Это сохраняет компактность таблицы и одновременно даёт проверяемость.

### 19.7 Исключение: методологические константы

Формулу НЕ добавляем только для метрик, которые не являются производными:
- `headcount`, `shifts_count`, `days` — это просто счётчики
- Нормы и бенчмарки (`norm.min`, `industry_staff_food_pct`) — константы

Для всех остальных — формула **обязательна**.

### 19.8 Применение в пайплайне n8n → ClickHouse (Phase 2.9.1)

SQL-запросы должны возвращать не только финальные проценты, но и их числители/знаменатели. Пример:

```sql
SELECT
  SUM(payroll_rub) AS payroll_numerator,
  SUM(revenue_rub) AS payroll_denominator,
  SUM(payroll_rub) * 100.0 / NULLIF(SUM(revenue_rub), 0) AS payroll_pct_value
FROM mart_staff_daily
WHERE ...
```

Worker потом упаковывает это в объект `{value, numerator, denominator, formula, unit}` и возвращает.

---

## Итоговая структура ответа `/api/staff-list` (v3)

```json
{
  "period": { "start": "2026-03-01", "end": "2026-04-20", "days": 51 },
  "period_stats": { /* абсолюты — новое в v3 */ },
  "kpi": {
    "payroll_pct_of_revenue": { "value": 13.0, "numerator": ..., "denominator": ..., "formula": "...", "unit": "%", "norm": {...} },
    "active_headcount": { "value": 24, "formula": "...", "unit": "чел" },
    "revenue_per_hour_rub": { "value": 2832, "numerator": ..., "denominator": ..., "formula": "...", "unit": "₽/час" },
    "rotation_pct": { "value": 4.2, "numerator": 1, "denominator": 24, "formula": "...", "unit": "%" },
    "days_without_manager": { "value": 0, "numerator": 0, "denominator": 51, "formula": "...", "unit": "дн" }
  },
  "employees": [ /* массив без left-сотрудников */ ],
  "summary": {
    "active_headcount": 24,
    "total_hours": 4290,
    "total_payroll": 1578340,
    "status_counts": { "core": 12, "regular": 9, "new": 2, "occasional": 1, "dormant": 0 },
    "group_counts": { "Кухня": 11, "Зал": 7, "Бар": 2, "Клининг": 3, "-": 2 },
    "excluded_left_count": 1
  },
  "meta": {
    "mock": true,
    "pipeline_status": "Phase 2.9.2b — mock scales with period",
    "payroll_data_valid_from": "2024-07-01",
    "left_staff_threshold_days": 21
  }
}
```
