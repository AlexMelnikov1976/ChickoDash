...
**Последнее обновление:** 17.04.2026, 23:40 MSK — Волна 1 инфраструктуры завершена
**Версия паспорта:** 3.5 (консолидирует v3.3, v3.4 + результаты 17.04 вечер)
...
| **Общий прогресс** | ~35% от плана (Волна 1 инфры ✅, API 75%, Dashboard 0%) |
...
├── .github/
│   └── workflows/
│       └── deploy.yml    # GitHub Actions (TBD, Волна 2)
├── .gitignore
...
Сгруппировано по приоритету. **Не параллелить внутри волн** — порядок важен, последующие полагаются на предыдущие.
...
| Шаг | Время | Экономия |
|---|---|---|
| GitHub Actions workflow `.github/workflows/deploy.yml` — автодеплой на push в main | ~40 мин | 3-5 мин × каждый deploy |
| Cloudflare API Token → GitHub Secrets | ~10 мин | Часть выше |
...
| **Ротация ClickHouse пароля** — полный цикл (Yandex → менеджер паролей → wrangler → n8n → `.dev.vars`) | ~15 мин | Закрытие security-риска |
| **Активация n8n proxy** — импорт workflow, активация, update `CLICKHOUSE_HOST` secret | ~20 мин | Разблокировка `/api/query` |
...
1. **n8n proxy не активирован** — workflow JSON готов, но ещё не импортирован и не активирован. Блокирует `/api/query` end-to-end. ETA: Волна 2.
2. **ClickHouse пароль не ротирован** — лежит в старом HTML v4 в открытом виде. Security-риск. ETA: Волна 2.
3. **Нет автодеплоя** — каждый `wrangler deploy` делается руками. Риск забыть deploy после коммита. ETA: Волна 2.
...
- **Как задеплоить код** → [README.md — Deployment](../README.md#deployment)
...
**Версии паспорта:** v3.3 → v3.4 → **v3.5** (текущая, консолидирует всё до 17.04.2026 вечера)
