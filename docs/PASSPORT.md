...
**Последнее обновление:** 22.04.2026, ~12:30 MSK — Score v2.0: гибрид «Здоровье (60%) + Рост YoY (40%)». Блок Рост сравнивает ресторан с самим собой за тот же период прошлого года (fallback: 90-дневная медиана). Блок Здоровье — абсолютные KPI (фудкост, скидки, баланс доставки). Маржа убрана (модельные данные ненадёжны для скора). Плашка рейтинга на Обзоре рядом с прогнозом, тултип «Как считается?», кликабельная таблица всех ресторанов. Ранее в этой сессии: fix #77 login form, Phase 2.5 user activity log (CH + trackUI). Данные `dish_sales` 870К строк загружены.
**Версия паспорта:** 3.33
...
| **Последний коммит** | Score v2.0 (YoY growth + health KPIs) + Phase 2.5 activity log + fix #77 login form |
...
| **Следующий фокус** | 22.04: вкладка «Меню» (Kasavana-Smith). 28.04: CSP enforce. Май: пилот с 3-5 франчайзи. Хвосты: UI logout button, select_restaurant tracking, score v2 калибровка по фидбеку |
...
### 22.04.2026, день-6 (~4 часа работы) — fix #77 + Phase 2.5 activity log + Score v2.0

**Контекст:** Презентация франчайзи. Три блокера: (1) кнопка логина не работает; (2) нет истории работы пользователей; (3) скор убран с главной, нужен обратно с рейтингом. Параллельно за ночь загружены данные по блюдам (`dish_sales`, 870К строк).

#### Fix #77 — login form click broken

**Причина:** `DOMContentLoaded` внутри template literal — к моменту выполнения скрипта DOM уже ready, listener не ставился. **Фикс:** inline `onsubmit="return handleLoginSubmit(event)"` + глобальная функция.

#### Phase 2.5 — User activity log

Таблица `chicko.user_activity_log` в ClickHouse (MergeTree, TTL 365 дней). Серверный трекинг 6 endpoints через `_logReq()` + `waitUntil`. Клиентский `trackUI()` → `POST /api/activity`. Файлы: `activity_log.ts` (новый), `index.ts`, `dashboard.ts`.

#### Score v2.0 — «Здоровье + Рост YoY»

**Методология:** гибрид из двух блоков, разработана после анализа двух внешних аудитов текущей формулы.

**Блок «Рост» (40%)** — динамика ресторана к самому себе:
- Выручка (20%): текущий avg / YoY avg × 80. На уровне прошлого года = 80, рост +25% = 100
- Чеки (10%): аналогично
- Ср. чек (10%): аналогично
- База: тот же период прошлого года (≥7 дней). Fallback: 90-дневная медиана

**Блок «Здоровье» (60%)** — абсолютные KPI, одинаковые для всех:
- Фудкост (25%): кусочно-линейная шкала (норма 18-21%, прогрессивный штраф выше)
- Скидки (20%): `clamp(0, 100, 100 − discount × 5)`
- Баланс доставки (15%): `clamp(0, 100, 100 − |share − 20| × 3)`, оптимум 20%

**Что убрано vs v1:** маржа (модельные 40% расходов ненадёжны), TOP10 как база (заменён на YoY), «Трафик» переименован в «Чеки».

**Что решает:** маленький ресторан с хорошей динамикой и здоровым фудкостом получит высокий скор. Сезонность нейтрализована через YoY.

**UI:** плашка рейтинга рядом с прогнозом на одной горизонтали. Тултип «Как считается?» с описанием формулы. Таблица всех ресторанов (город, балл, цветовая полоска), клик переключает ресторан. Breakdown: 6 полосок в 2 группах (Рост YoY / Здоровье).

**Документация:** `docs/SCORE_METHODOLOGY_v2.md` — полная методология с обоснованием, путём калибровки и планом развития.

**Калибровка после пилота:** фидбек «ваш скор X — согласны?», корреляция с EBITDA после 1С, добавление CV-стабильности, сегментация по concept.

#### Данные по блюдам

Таблица `chicko.dish_sales` загружена: 870К строк, 539 уникальных блюд, 45 ресторанов, данные с 2024-01-02 по 2024-10-11. Реализация вкладки «Меню» + Kasavana-Smith — следующая задача
...
**Контекст:** В день-2 был упрощён P&L калькулятор (убраны захардкоженные ФОТ 25% / Аренда 15%). При работе с пользователем всплыло что текущий калькулятор даёт **путаные и некорректные** результаты — нашли и починили цепочку из 7 проблем подряд. Финальная версия (v4) — калькулятор выручки с честной логикой остатка месяца, без маржи (она уходит в P&L вкладку после 1С).

#### Хронология фиксов (7 итераций #76):

**v1 — упрощение (коммит `fix(#76): simplify P&L calculator → honest margin`):**
- Убраны захардкоженные `fot = net*0.25` и `rent = net*0.15`
- `profit` → `margin` (маржа до прочих расходов)
- Breakeven заменён на заглушку «после 1С»

**v2 B — двухсценарная схема (коммит `feat(#76 B): margin calculator with weekday/weekend split`):**
- Вместо одной колонки ползунков — две (Будни и Выходные) с 4 ползунками каждая
- Месячный итог: 22 будн × маржа будни + 8 вых × маржа выходные (было: 26 × средняя)
- Блок «Будни vs Выходные» сверху теперь согласован с калькулятором снизу

**v2 B polish-1 (коммит `polish(#76): step=1 for check slider, per-row deltas + revenue forecast`):**
- Ползунок чека: `step="10"` → `step="1"` (отображение до рубля)
- Дельты на каждой строке сценарной колонки
- Добавлена строка «Прогноз выручки/мес» в обе колонки

**v2 B polish-2 (коммит `polish(#76): display avg check in full format`):**
- Подписи чека: компактный `1.9К₽` → полный `1 897 ₽` (через `fmtR(v, true)`)

**Фикс связи город↔калькулятор (коммит `fix(#76): call renderAnalysis from renderAll`):**
- Баг: `renderAll` вызывал renderForecast/renderKPIs/renderDynamics/renderCompare, но НЕ renderAnalysis
- При смене города вкладка Анализ оставалась с данными предыдущего ресторана
- Фикс: добавлен `renderAnalysis` в `renderAll` с try/catch (как у Dynamics/Compare)

**v2 B v2 — 3 связанных фикса (коммит `fix(#76 B v2): 90-day baseline, exact slider sync, remaining-month logic`):**
- **Фикс A**: Baseline ползунков теперь из последних 90 дней истории R.ts, не из выбранного периода. Стабильно — не прыгает при смене фильтра.
- **Фикс B**: После `element.value = Math.round(chk)` читаем `.value` обратно в `S.plWd*`. Раньше S хранил точное значение (1867), ползунок — округлённое до step (1870). При сбросе эффект показывал -1.8К вместо 0.
- **Фикс C**: Прогноз месяца = `pastActualMargin + remWd × wd_margin + remWe × we_margin`. Прошедшие дни считаются по реальной марже из R.ts. Оставшиеся дни — по ползункам. Раньше считалось как «весь месяц с нуля» — неправильно если уже 20 число.

**v3 — чек уже netto, вариант C для скидок (коммит `fix(#76): chk is netto (no double discount), variant C disc slider semantics`):**
- **Критичный фикс**: в iiko `avg_check_total_rub` приходит уже за вычетом скидок. Прежний plCalc вычитал скидки повторно → занижал маржу на ~1% от нетто (разница `disc% × fc%`).
- Новая семантика ползунка скидок: если уменьшаем скидки, netto-чек растёт пропорционально (предположение: гросс-чек и поведение клиентов не меняются).
- Формула: `effective_netto = baseline_netto × (1 − new_disc/100) / (1 − baseline_disc/100)`
- При `disc == baselineDisc` → netFactor = 1 (нейтрально).
- `computePlContext.pastActualMargin` тоже исправлен — убран повторный вычет скидок.

**v4 — фокус на выручку (коммит `feat(#76 v4): revenue-focused Analysis tab with waterfall`):**
- Вся маржа вынесена в будущую отдельную P&L вкладку (после 1С)
- Убрано из Анализа: фудкост-ползунки, «Маржа/день», график накопленной маржи 30 дней, точка безубыточности, предзаданные сценарии улучшений, bar chart структуры нетто-выручки
- Блок «Разбор сценария» переделан в waterfall: аддитивно применяет изменения ползунков от baseline к сценарию, каждая строка показывает ЧТО было → ЧТО стало → эффект в ₽. Сумма шагов = общему эффекту.

#### Файлы

Только `src/dashboard.ts` (+~200 строк после всех итераций, с учётом удалённых). Все 7 коммитов последовательные, TypeScript `--noEmit` чист после каждого.

#### Обоснование подхода «маржу в P&L после 1С»

Проблемы которые всплыли: путаница между «маржой» и «выручкой» в UI, невозможность показать «в карман» без реальных ФОТ/аренды, фудкост-ползунок без контекста ФОТ не даёт полной картины. Решение: лист Анализ отвечает на вопрос **«сколько выручки принесёт сценарий»**, а отдельная P&L-вкладка (после 1С) — **«сколько в карман после всех затрат»**. Это чистое разделение ролей: выручка = from iiko, маржа/прибыль = from iiko + 1С.
...
- **UX-проход (~2ч):** #23 шрифты, #25 цветовая слепота, #26 1366px, ~~#43 доставка 0%~~ ✅ закрыто, ~~#48 формат денег~~ ✅ закрыто, #51 tooltip anomaly_day
- ~~**Мелочь замеченная в проде:** дубликат названия в заголовке прогноза~~ — ✅ закрыто 21.04

**Волна 6 (см. раздел 12):**
- Модуль анализа продаж по блюдам (15-20ч)
- **#78 (новое) — Отдельная вкладка P&L (~8-12ч после подключения 1С):**
  - Полный расчёт прибыли с реальными ФОТ, арендой, коммуналкой, прочими затратами
  - Фудкост как ползунок (сейчас вынут из Анализа)
  - Точка безубыточности с настоящими постоянными затратами
  - Структура затрат (stacked bar: скидки + фудкост + ФОТ + аренда + прочее + прибыль)
  - Интеграция с #77 (1С Калининграда → ClickHouse)
  - Предусловие: Волна 6 1С-интеграция должна быть готова
...
**Версии:** v3.3 → ... → v3.30 → v3.31 → v3.32 → **v3.33** (Score v2.0: Здоровье 60% + Рост YoY 40%, маржа убрана, кусочно-линейный фудкост, баланс доставки. Плашка рейтинга рядом с прогнозом + тултип + кликабельная таблица. Fix #77, Phase 2.5 activity log. 22.04.2026, день-6)
