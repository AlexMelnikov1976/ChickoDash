...
**Последнее обновление:** 22.04.2026, ~15:00 MSK — Phase 2.6 AI insight задеплоен в прод (кнопка «🤖 AI-анализ» на Обзоре → `/api/ai-insight` → Claude Sonnet → JSON с тремя директорами + actions). После успешного smoke-test кнопка временно отключена (`disabled title="Скоро будет доступно"`) для контроля расходов до пилота. Phase 2.7 backend `/api/menu-analysis` с Kasavana-Smith классификацией задеплоен и протестирован (134 блюда, 41 star / 31 plowhorse / 30 puzzle / 32 dog). UI вкладки «Меню» — 2 попытки с разными подходами (Python + sed), обе ломали dashboard.ts через невидимые ошибки, откатывались через `git checkout -- src/dashboard.ts`. **Причина провала:** template literal с кириллицей + unicode escapes + многострочные правки = слишком хрупкая комбинация для sed/awk. Нужен другой подход — переписать dashboard.ts как внешний HTML-файл, загружаемый через `fetch`, либо разбить на модули.
**Версия паспорта:** 3.34
...
| **Последний коммит** | Phase 2.6 AI insight (кнопка временно disabled) + Phase 2.7 menu-analysis backend. UI меню — пока заблокирован архитектурой dashboard.ts |
...
| **Следующий фокус** | Разобрать архитектуру dashboard.ts (рефакторинг template literal → модульный HTML) для безопасного добавления UI меню. Презентация франчайзи сегодня с доступными фичами (скор, прогноз, сравнение, активити-лог). 28.04: CSP enforce. Май: пилот |
...
### 22.04.2026, день-6 (~7 часов работы) — fix #77 + Phase 2.5/2.6/2.7 + Score v2.0 + урок про dashboard.ts

#### Итог дня — что работает в проде

1. **Fix #77** — login form работает, inline onsubmit
2. **Phase 2.5** — user_activity_log в CH, серверный + клиентский трекинг (`trackUI`, `/api/activity`), 6 protected endpoints через `waitUntil`
3. **Score v2.0** — Здоровье (60%) + Рост YoY (40%), плашка рейтинга рядом с прогнозом, breakdown в 2 группах, клик по ресторану в таблице → переключение
4. **Phase 2.6 AI insight** — `/api/ai-insight` + Claude Sonnet через `ANTHROPIC_API_KEY` (wrangler secret), три директора + actions, rate limit 5/5мин. Smoke-test OK. **Кнопка временно отключена** для контроля расходов до пилота.
5. **Phase 2.7 backend menu-analysis** — `/api/menu-analysis` с Kasavana-Smith классификацией. Протестирован: 134 блюда в Калининграде за период 2024-06, корректная классификация (41 star / 31 plowhorse / 30 puzzle / 32 dog).

#### Что не вышло — UI меню

Две попытки добавить вкладку Меню в dashboard.ts, обе откатились.

**Попытка 1** (Python-скрипт с triple-quoted strings): ошибки в unicode escape последовательностях, `\u{1F9E9}` превращался в `<0001f9e9>` в JS-строках, таблица не рендерилась.

**Попытка 2** (sed + awk по одной правке): избежали большинства unicode-проблем, получилось 5/5 патчей с чистым typecheck, но в проде **опять не рендерилось** — при `git checkout -- src/dashboard.ts` правки исчезли вместе с отключением AI-кнопки (которое не было закоммичено).

**Корневая причина:** dashboard.ts — один файл 3400+ строк, template literal с кириллицей + HTML + JS + CSS + эмодзи + escape-последовательностями. Любая многострочная правка через sed/awk/python с высокой вероятностью сломает backtick-экранирование или template literal boundary, и это не ловится tsc (template literal валиден как строка, но внутри может быть битый JS).

**Правильный путь:**
- Вариант A: вынести HTML/JS из template literal во внешние файлы (`dashboard.html`, `dashboard.js`), воркер отдаёт их отдельно
- Вариант B: использовать Vite/esbuild для сборки, template literal генерируется автоматически
- Вариант C: разбить DASHBOARD_HTML на модули (header, panels, js) через отдельные export const

Для текущей сессии решено: остановиться, презентовать франчайзи то что работает, архитектурный рефакторинг — отдельная задача.

#### Fix #77 — login form click broken

Причина: `DOMContentLoaded` внутри template literal — к моменту выполнения скрипта DOM уже ready. Фикс: inline `onsubmit="return handleLoginSubmit(event)"` + глобальная функция.

#### Phase 2.5 — User activity log

`chicko.user_activity_log` в ClickHouse (MergeTree, TTL 365 дней). Серверный `_logReq()` + `waitUntil` на 6 endpoints. Клиентский `trackUI()` → `POST /api/activity`. Файлы: `activity_log.ts` (новый), `index.ts`, `dashboard.ts`.

#### Score v2.0 — «Здоровье + Рост YoY»

**Блок «Рост» (40%)** — динамика к себе через YoY:
- Выручка (20%), Чеки (10%), Ср. чек (10%)
- База: тот же период прошлого года (≥7 дней), fallback: 90-дневная медиана

**Блок «Здоровье» (60%)** — абсолютные KPI:
- Фудкост (25%, кусочно-линейная 18-21%), Скидки (20%), Баланс доставки (15%, оптимум 20%)

**Убрано:** маржа (модельные 40% расходов ненадёжны). **UI:** плашка рядом с прогнозом, тултип «Как считается?», кликабельная таблица.

**Документация:** `docs/SCORE_METHODOLOGY_v2.md`

#### Phase 2.6 — AI Insight

**Архитектура:** клиент собирает JSON (KPI + YoY + медианы сети) → `POST /api/ai-insight` → сервер вызывает Claude Sonnet с system prompt «Ты совет директоров» → возвращает JSON {operations, finance, commercial, actions[]} → рендерится в три карточки + список действий.

**Безопасность:** `ANTHROPIC_API_KEY` в wrangler secret, не в клиенте. Rate limit 5 req / 5 min per user.

**Файлы:** `ai_insight.ts` (новый), `index.ts`, `dashboard.ts`.

**Пример ответа из prod smoke-test** (Калининград-1, апрель 2026, скор 58):
> Операционный директор: «Критическое падение трафика на 40.5%... При 73 чеках против 94 по сети загрузка крайне низкая»
> Финансовый директор: «Фудкост 22% превышает норму... Падение выручки на 45.4% к прошлому году...»
> Actions: (1) агрессивная акция трафика, (2) пересмотр меню для снижения FC до 20%, (3) маркетинг доставки до 35%

#### Phase 2.7 backend — Menu Analysis

**Endpoint:** `GET /api/menu-analysis?restaurant_id=N&start=...&end=...`
**Логика Kasavana-Smith:**
- popularity threshold = (1/N) × 0.70 × 100
- profitability threshold = weighted avg margin per unit
- 4 класса: Stars / Plowhorses / Puzzles / Dogs

**Файлы:** `menu_analysis.ts` (новый, запушен в коммите `64e274c`), `index.ts`.

**Статус:** backend работает, UI не добавлен (см. выше «Что не вышло»).

### 22.04.2026, день-6 (~4 часа работы — ранняя часть сессии, заменено расширенной версией выше) — fix #77 + Phase 2.5 user activity log + Score v2.0
...
**Версии:** v3.3 → ... → v3.32 → v3.33 → **v3.34** (Phase 2.6 AI insight задеплоен (кнопка временно disabled); Phase 2.7 backend menu-analysis с Kasavana-Smith задеплоен; UI меню не добавлен из-за архитектурной хрупкости template literal в dashboard.ts — требуется рефакторинг. 22.04.2026, день-6 ~17:00 MSK)
